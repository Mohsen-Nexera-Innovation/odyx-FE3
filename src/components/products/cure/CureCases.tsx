...
export default function CureCases() {
  ...
  return (
    <div className="reveal flex h-full flex-col rounded-[18px] border-4 border-solid border-white bg-transparent p-[clamp(16px,1.8vw,22px)]">
      ...
      <div
        className="cure-case-fade mb-[18px] flex items-center justify-between gap-0 [animation:p126-fade_.35s_ease] max-[640px]:flex-col"
        key={tab.id}
        role="tabpanel"
      >
        {tab.steps.map((step, i) => (
          <Fragment key={step.label}>
            <div className="group flex min-w-0 flex-[1_1_0] flex-col items-center gap-2 text-center">
              ...
              <p className={`${SANS} m-0 text-[.78rem] font-semibold leading-[1.25] text-[#2c3444]`}>
                {step.label}
              </p>
            </div>
