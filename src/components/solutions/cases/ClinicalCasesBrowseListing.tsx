...
import ProductCaseCardView from '@/components/solutions/cases/products/ProductCaseCard';
...
export type ClinicalCasesBrowseTab<Id extends string> = {
  id: Id | 'all';
  label: string;
};
...
type Props<Id extends string> = {
  ...
  activeId: Id | 'all';
  tabs: ClinicalCasesBrowseTab<Id>[];
  pathFor: (id: Id | 'all') => string;
  sectionTitle: (id: Id) => string;
  filterItems: (cases: ProductCaseCard[], id: Id | 'all', query: string) => ProductCaseCard[];
  cardHref: (item: ProductCaseCard, listingId: Id | 'all') => string | null;
  tablistLabel: string;
};
...
function CaseGrid({
  ...
  items: ProductCaseCard[];
  hrefFor: (item: ProductCaseCard) => string | null;
}) {
  return (
    <ul className="m-0 p-0 list-none [display:grid] grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
      {items.map((item) => (
        <li key={item.id} className="min-w-0">
          <ProductCaseCardView item={item} href={hrefFor(item)} />
        </li>
      ))}
    </ul>
  );
...
function CaseSwiper({
  ...
  const prevClass = `clinical-cases-prev-${navId}`;
  const nextClass = `clinical-cases-next-${navId}`;
...
export default function ClinicalCasesBrowseListing<Id extends string>({
  cases,
  activeId,
  tabs,
  pathFor,
  ...
  const familyIds = tabs.map((tab) => tab.id).filter((id): id is Id => id !== 'all');

  const sections = useMemo(() => {
    const ids: Id[] = isAll ? familyIds : [activeId as Id];
    return ids
      .map((id) => ({
        id,
        title: sectionTitle(id),
        items: filterItems(cases, id, q),
      }))
      .filter((s) => s.items.length > 0);
  }, [cases, familyIds, filterItems, isAll, activeId, q, sectionTitle]);
    ...
    <div className="bg-[#F3F6FA] w-full min-h-dvh overflow-x-clip pb-12" data-hero-light>
      <div className={`${GUTTER} pt-[calc(var(--hdr-h)+20px)] sm:pt-[calc(var(--hdr-h)+28px)] lg:pt-[calc(var(--hdr-h)+40px)]`}>
        <div className="flex flex-col items-center text-center mb-7 md:mb-10 min-w-0">
          ...
          <h1
            id="clinical-cases-browse-title"
        ...
        {sections.length === 0 ? (
          ...
          <div className="flex flex-col gap-8 md:gap-9">
            {sections.map((section) => (
              <section key={section.id} className="min-w-0" aria-labelledby={`clinical-cases-${section.id}`}>
                <div className="flex items-center justify-between gap-2 sm:gap-3 mb-3 min-w-0">
                  <h2
                    id={`clinical-cases-${section.id}`}
                ...
                {isAll ? (
                  <CaseSwiper
                    items={section.items}
                    hrefFor={(item) => cardHref(item, section.id)}
                    navId={section.id}
                  />
                ) : (
                  <CaseGrid items={section.items} hrefFor={(item) => cardHref(item, activeId)} />
                )}
              </section>
            ))}
          </div>
